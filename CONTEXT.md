...
| **Role chosen at signup, inside the auth form** | Was a separate screen before the signup form; changed later so login — the common case — is what a returning user sees first, with no screen in front of it. Role is now asked inline within the signup form itself, and still travels in the signup request with the server treating it as fixed. |
