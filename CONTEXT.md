...
have broken the Railway deploy. `scripts` is excluded from `tsconfig.build.json`.

**The lockfile must be generated by the same npm the build host uses.** Render installs the npm
bundled with `.node-version` (Node 22 → npm 10). npm 11 nests `@emnapi/*` under
...
| Railway / Render | Not created. Needed for DEMO-20. |
...
**DEMO-20 — backend deploy.** Railway or Render, pointed at the same Supabase database. The env vars
