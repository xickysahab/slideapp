...
Generated from the running server's route table and the live database schema, not from memory —
if something here disagrees with the code, the code is right and this file needs fixing.
...
Deliberately **not** implemented, per §2 of that doc: RLS policies, malware scanning, rate limiting,
recruiter verification, DPDP consent flows, penetration testing. These are what stand between this
build and a public link.
